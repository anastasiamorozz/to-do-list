...
class RoomContoller { 
  ...
  async addUserToRoom(req, res) { 
    ...
    try { 
      ...
      const checkUserInRoom = await db.query( 
        "SELECT * FROM UsersInRoom WHERE user_id = $1 AND room_id = $2", 
        [userId, roomId] 
      ); 
      ...
      const addUserQuery = 
        "INSERT INTO UsersInRoom (user_id, room_id) VALUES ($1, $2)"; 
  ...
  async deleteRoom(req, res) { 
    ...
    try { 
      const result = await db.query("DELETE FROM Rooms WHERE room_id = $1", [ 
        roomId, 
      ]); 
  ...
  async getTasksInRoom(req, res) { 
    ...
    try { 
      const tasks = await db.query( 
        `SELECT * FROM  Task WHERE room_id=$1 ORDER BY "day" ASC`, 
        [roomId] 
      ); 
  ...
  async getUsersInRoom(req, res) { 
    ...
    try { 
      ...
      WHERE ur.room_id = $1 
        AND u.id != (SELECT creator_id FROM Rooms WHERE room_id = $1) 
    `; 
  ...
  //-------------------------------------------------------------------- 
  async deleteUserInRoom(req, res) { 
    const roomId = req.params.roomId; 
    const userId = req.params.userId; 
    try { 
      const queryText = `DELETE FROM UsersInRoom WHERE user_id = $2 AND room_id = $1`; 
      await db.query(queryText, [roomId, userId]); 
      res.sendStatus(200); 
    } catch (error) { 
      console.error("Error deleting user in room:", error.message); 
      res.status(500).json({ error: "Internal Server Error" }); 
    } 
  } 
} 
